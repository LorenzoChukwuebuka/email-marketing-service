...
import (
	...
	"email-marketing-service/core/handler/domain/mapper"
...
func NewDomainService(store db.Store) *DomainService {
	return &DomainService{
		store: store,
	}
}

func (s *DomainService) CreateDomain(ctx context.Context, req *dto.DomainDTO) (any, error) {
	...
	domainExists, err := s.store.CheckDomainExists(ctx, db.CheckDomainExistsParams{
		Domain:    req.Domain,
	...
	fmt.Printf("domainExists :%v", domainExists)

	verifyDomain := s.verifyDomain(req.Domain)
	if !verifyDomain {
		return nil, fmt.Errorf("your domain is not verified")
	}

	// Generate DNS records
	txtRecord := s.generateTXTRecord(req.Domain)
	dmarcRecord := s.generateDMARCRecord(req.Domain)
	dkimSelector := s.generateDKIMSelector()
	dkimPublicKey, dkimPrivateKey, err := s.generateDKIMKeys()
	spfRecord := s.generateSPFRecord(req.Domain)
	mxRecord := s.generateMXRecord(req.Domain)
	if err != nil {
	...
	data := db.CreateDomainParams{
		UserID:         _uuid["user"],
		CompanyID:      _uuid["company"],
		TxtRecord:      sql.NullString{String: txtRecord, Valid: true},
		DmarcRecord:    sql.NullString{String: dmarcRecord, Valid: true},
		DkimPublicKey:  sql.NullString{String: dkimPublicKey, Valid: true},
		DkimPrivateKey: sql.NullString{String: dkimPrivateKey, Valid: true},
		DkimSelector:   sql.NullString{String: dkimSelector, Valid: true},
		MxRecord:       sql.NullString{String: fmt.Sprintf("mail.%s", req.Domain), Valid: true},
		SpfRecord:      sql.NullString{String: spfRecord, Valid: true},
		Domain:         req.Domain,
	}

	//create the domain
	_, err = s.store.CreateDomain(ctx, data)
	if err != nil {
		return nil, common.ErrCreatingRecord
	}

	// Generate downloadable content
	downloadContent, err := s.generateDownloadableRecords(req.Domain, dnsRecords)
	if err != nil {
...
func (s *DomainService) generateTXTRecord(domain string) string {
	...
	hash := sha256.Sum256([]byte(domain + cfg.ENC_KEY))
...
func (s *DomainService) generateDKIMKeys() (string, string, error) {
	...
	privateKey, err := rsa.GenerateKey(rand.Reader, common.Keysize)
	if err != nil {
		return "", "", err
	}
...
func (s *DomainService) InitiateVerification(ctx context.Context, req *dto.FetchDomainDTO) (bool, error) {
	_uuid, err := common.ParseUUIDMap(map[string]string{
		"company": req.CompanyID,
		"domain":  req.DomainID,
	})
	...
	if err != nil {
		return false, common.ErrInvalidUUID
	}

	domain, err := s.store.GetDomainByIDAndCompany(ctx, db.GetDomainByIDAndCompanyParams{
		ID:        _uuid["domain"],
		CompanyID: _uuid["company"],
	})
	if err != nil {
	...
	if verified {
		err = s.store.UpdateDomain(ctx, db.UpdateDomainParams{
			ID:        _uuid["domain"],
			CompanyID: _uuid["company"],
...
func (s *DomainService) VerifyDNSRecords(domain db.Domain) (bool, error) {
	txtVerified, err := s.verifyTXTRecord(domain.Domain, domain.TxtRecord.String)
	if err != nil {
		return false, err
	}
	...
	dmarcVerified, err := s.verifyDMARCRecord(domain.Domain, domain.DmarcRecord.String)
	if err != nil {
		return false, err
	}
	...
	dkimVerified, err := s.verifyDKIMRecord(domain.Domain, domain.DkimSelector.String, domain.DkimPublicKey.String)
	if err != nil {
		return false, err
	}
	...
	mxVerified, err := s.verifyMXRecord(domain.Domain, domain.MxRecord.String)
	if err != nil {
		return false, err
	}
	...
	spfVerified, err := s.verifySPFRecord(domain.Domain, domain.SpfRecord.String)
	if err != nil {
		return false, err
	}
	if !spfVerified {
...
func (s *DomainService) DeleteDomain(ctx context.Context, req *dto.FetchDomainDTO) error {
	_uuid, err := common.ParseUUIDMap(map[string]string{
		"company": req.CompanyID,
		"domain":  req.DomainID,
	})
	...
	if err := s.store.SoftDeleteDomain(ctx, db.SoftDeleteDomainParams{
		CompanyID: _uuid["company"],
		ID:        _uuid["domain"],
	}); err != nil {
		return err
...
func (s *DomainService) GetDomain(ctx context.Context, req dto.FetchDomainDTO) (any, error) {
	_uuid, err := common.ParseUUIDMap(map[string]string{
		"company": req.CompanyID,
		"domain":  req.DomainID,
	})
	...
	data := mapper.MapDomain(domain)
	return data, nil
}

func (s *DomainService) GetAllDomains(ctx context.Context, req dto.FetchDomainDTO) (any, error) {
	_uuid, err := common.ParseUUIDMap(map[string]string{
		"company": req.CompanyID,
	})
	if err != nil {
	...
	domain, err := s.store.ListDomainsByCompany(ctx, db.ListDomainsByCompanyParams{
		CompanyID: _uuid["company"],
	...
	domain_count, err := s.store.CountDomainByCompanyID(ctx, _uuid["company"])
	if err != nil {
		return nil, common.ErrFetchingRecord
	}

	data := mapper.MapDomains(domain)
	...
	response := common.Paginate(int(domain_count), items, int(req.Limit), int(req.Offset))
	return response, nil
}
