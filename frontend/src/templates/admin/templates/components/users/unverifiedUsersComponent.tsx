...
const UnVerifiedUsersTable: React.FC = () => {
    ...
    const handlePageChange = (newPage: number) => {
        getUnverifiedUsers(newPage, paginationInfo.page_size);
    };
    ...
    const handleToggle = async (userId: string, field: 'verified' | 'blocked') => {
        ...
        try {
            ...
            useAdminUserStore.getState().getUnverifiedUsers();
        }
    };
    ...
    useEffect(() => {
        if (debouncedSearchQuery !== "") {
            searchUser(debouncedSearchQuery,"Unverified");
        } else {
            getUnverifiedUsers(); // Reset to all contacts when search query is empty
        }
    }, [debouncedSearchQuery, searchUser, getUnverifiedUsers]);
