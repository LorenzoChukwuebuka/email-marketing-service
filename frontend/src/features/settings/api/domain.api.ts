...
class DomainAPI {
    static async createDomain(domainValues: DomainFormValues): Promise<ResponseT> {
        const response = await axiosInstance.post<ResponseT>("/domain/create-domain", domainValues)
        return response.data
    }

    static async deleteDomain(uuid: string): Promise<ResponseT> {
        const response = await axiosInstance.delete<ResponseT>("/domain/delete-domain/" + uuid)
        return response.data
    }

    static async authenticateDomain(uuid: string): Promise<ResponseT> {
        const response = await axiosInstance.put<ResponseT>("/domain/authenticate-domain/" + uuid)
        return response.data
    }

    static async getDomain(uuid: string): Promise<APIResponse<DomainRecord>> {
        const response = await axiosInstance.get<APIResponse<DomainRecord>>("/domain/get-domain/" + uuid)
        return response.data
    }

    static async getAllDomains(page?: number, pageSize?: number, query?: string): Promise<APIResponse<PaginatedResponse<DomainRecord[]>>> {
        const response = await axiosInstance.get<APIResponse<PaginatedResponse<DomainRecord[]>>>("/domain/get-all-domains", {
            params: {
                page: page,
                page_size: pageSize,
                search: query
