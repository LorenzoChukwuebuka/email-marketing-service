...
const AllSupportTicketComponentTable: React.FC = () => {
    ...
    const { data: supportData, isLoading } = useAdminAllTicketsQuery(currentPage, pageSize, debouncedSearchQuery)
                            ...
                            supportData.map((ticket, index) => {
                                return (
                                    <tr key={index} className="hover:bg-gray-100">
                                        ...
                                        <td className="py-4 cursor-pointer px-4" onClick={() => navigate("/zen/support/details/" + ticket.uuid)}> <i className="bi bi-eye"></i> </td>
                                    </tr>
                                );
                            })
                        ) : (
